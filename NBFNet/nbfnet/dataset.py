...
@R.register("datasets.RDKG")
class RDKG(data.KnowledgeGraphDataset):
    ...
    def __init__(self, path, verbose=1):
        ...
        target_file = os.path.join(path, "train_targets.txt")

        for f in txt_files + [target_file]: 
            if not os.path.exists(f):
                raise FileNotFoundError(f)
            
        self.load_tsvs(txt_files, verbose=verbose)
        n_train = self.num_samples[0]
        triple_to_idx = {}
        for i in range(n_train):
            h, t, r = self[i].tolist()
            triple_to_idx[(h,t,r)] = i

        target_indices = []
        missing = 0
        with open(target_file, "r") as f:
            for line in f: 
                h_tok, r_tok, t_tok = line.strip("\n").split("\t")
                h = self.inv_entity_vocab.get(h_tok)
                t = self.inv_entity_vocab.get(t_tok)
                r = self.inv_relation_vocab.get(r_tok)
                if h is None or t is None or r is None: 
                    missing += 1
                    continue 
                idx = triple_to_idx.get((h,t,r)) 
                if idx is None: 
                    missing += 1
                    continue
                target_indices.append(idx)
        if missing: 
            print("RDKG: warning -- %d target rows not found in train.txt "
                  "(vocab or tiple mismatch)" % missing)
        self.train_target_indices = target_indices 
    
    def split(self):
      n_train, n_valid, n_test = self.num_samples
      train_set = torch_data.Subset(self, self.train_target_indices)
      valid_set = torch_data.Subset(
          self, range(n_train, n_train + n_valid))
      test_set = torch_data.Subset(
          self, range(n_train + n_valid, n_train + n_valid + n_test))

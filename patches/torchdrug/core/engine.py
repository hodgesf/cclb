...
class Engine(core.Configurable):
    ...
    def load(self, checkpoint, load_optimizer=True, strict=True):
        ...
        state = torch.load(checkpoint, map_location=self.device, weights_only=False)
          ...
          # Drop non-tensor buffers (TorchDrug stores Graph objects under
          # 'graph' / 'fact_graph'; PyTorch 2.x's load_state_dict can't handle
          # non-tensor state_dict entries. They were already rebuilt from the
          # dataset during preprocess(), so the checkpoint copy isn't needed.)
        model_state = {k: v for k, v in state["model"].items() if torch.is_tensor(v)}
